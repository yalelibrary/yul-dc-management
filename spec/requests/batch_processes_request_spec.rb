...
RSpec.describe "BatchProcesses", type: :request, prep_metadata_sources: true do
  ...
  describe "GET /download_template" do
    ...
    it "downloads templates for reassociate" do
      ...
      expect(response).to have_http_status(:success)
      expect(response.content_type).to eq("text/csv; charset=utf-8")
      expect(response.body).to match("\xEF\xBB\xBFchild_oid,parent_oid,order,parent_title,label,caption,viewing_hint\nchild_oid,parent_oid,order,parent_title,call_number,label,caption,viewing_hin")
    end
  end
end
