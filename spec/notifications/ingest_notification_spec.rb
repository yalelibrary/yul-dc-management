...
RSpec.describe IngestNotification, prep_metadata_sources: true do
  ...
  describe '#to_database' do
    it 'saves the type and params to the database' do
      IngestNotification.with(parent_object: '99977711', status: 'failed', reason: 'Metadata Cloud did not return json').deliver(User.first)
  ...
  describe '#deliver_all' do
    it 'can be delivered to all users' do
      ...
      IngestNotification.with(parent_object: '99977722', status: 'failed', reason: 'Metadata Cloud did not return json').deliver_all
  ...
  describe '#message' do
    ...
    it 'returns the oid, status and reason' do
      IngestNotification.with(parent_object: parent_object, status: 'failed', reason: 'Metadata Cloud did not return json').deliver_all
