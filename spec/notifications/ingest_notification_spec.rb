...
RSpec.describe IngestNotification, prep_metadata_sources: true do
  ...
  describe '#to_database' do
    it 'saves the type and params to the database' do
      described_class.with(parent_object_id: '99977711', status: 'failed', reason: 'Metadata Cloud did not return json', batch_process_id: 'an_id').deliver(User.first)
  ...
  describe '#deliver_all' do
    it 'can be delivered to all users' do
      ...
      described_class.with(parent_object_id: '99977722', status: 'failed', reason: 'Metadata Cloud did not return json', batch_process_id: 'an_id').deliver_all
  ...
  describe '#message' do
    ...
    it 'returns the oid, status and reason' do
      described_class.with(parent_object_id: parent_object.id, status: 'failed', reason: 'Metadata Cloud did not return json', batch_process_id: 'an_id').deliver_all
