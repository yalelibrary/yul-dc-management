...
RSpec.describe DeleteParentObjectsJob, type: :job do
  before do
  ...
  let(:delete_parent_objects_job) { DeleteParentObjectsJob.new }

  context 'when delete_parent_objects raises an exception' do
    let(:error_message) { 'Something went wrong' }
    let(:error) { StandardError.new(error_message) }

    before do
      allow(batch_process).to receive(:delete_parent_objects).and_raise(error)
