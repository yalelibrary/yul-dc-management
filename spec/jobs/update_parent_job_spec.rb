...
RSpec.describe UpdateParentObjectsJob, type: :job, prep_metadata_sources: true, prep_admin_sets: true do
  ...
  let(:update_batch_process) { FactoryBot.create(:batch_process, user: user, file: create_many, batch_action: 'update parent objects') }
  let(:bare_update_batch_process) { FactoryBot.create(:batch_process, batch_action: 'update parent objects', user: user) }
  ...
  it 'increments the job queue by one' do
    update_parent_job = described_class.perform_later(bare_update_batch_process)
  ...
  context 'with more than limit of batch objects' do
    ...
    it 'goes through all parents in batches once' do
      ...
      expect(IngestEvent.where(status: 'update-complete').count).to eq total_parent_object_count
    end
  end
end
