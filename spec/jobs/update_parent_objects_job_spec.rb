...
RSpec.describe UpdateParentObjectsJob, type: :job do
  ...
  let(:update_parent_objects_job) { UpdateParentObjectsJob.new }

  context 'when update_parent_objects raises an exception' do
    let(:error_message) { 'Something went wrong' }
    let(:error) { StandardError.new(error_message) }

    before do
      allow(batch_process).to receive(:update_parent_objects).and_raise(error)
    ...
    it 'logs the error with batch_processing_event' do
      ...
      expect do
        # do not end the process when an error occurs. (raising error ends the process and we dont want that)
