...
RSpec.describe BatchProcess, type: :model, prep_metadata_sources: true do
  ...
  let(:csv_upload) { Rack::Test::UploadedFile.new(Rails.root.join(fixture_paths[0], "csv", "reassociation_example_small.csv")) }
  let(:child_object_nil_values) { Rack::Test::UploadedFile.new(Rails.root.join(fixture_paths[0], "csv", "reassociation_example_child_object_counts.csv")) }
  let(:child_object_caption_nil) { Rack::Test::UploadedFile.new(Rails.root.join(fixture_paths[0], "csv", "reassociation_example_child_caption.csv")) }
