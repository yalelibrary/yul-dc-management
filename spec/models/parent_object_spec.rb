...
RSpec.describe ParentObject, type: :model, prep_metadata_sources: true, prep_admin_sets: true do
  ...
  context "a newly created ParentObject with different visibilities" do
    ...
    let(:parent_object_owp) { described_class.create(oid: "12345", visibility: "Open with Permission", admin_set: FactoryBot.create(:admin_set)) }
    it "open with Permission visibility does not validate without a permission set" do
      expect(parent_object_owp.valid?).to eq false
    end

    let(:permission_set) { FactoryBot.create(:permission_set, label: 'set 1') }
    let(:parent_object_owp) { described_class.create(oid: "54321", visibility: "Open with Permission", admin_set: FactoryBot.create(:admin_set), permission_set: permission_set) }
