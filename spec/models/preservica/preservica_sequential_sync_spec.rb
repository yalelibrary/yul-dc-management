...
RSpec.describe Preservica::PreservicaObject, type: :model, prep_metadata_sources: true, prep_admin_sets: true do
  ...
  before do
    ...
    changing_fixtures.each do |fixture|
      stub_request(:get, "https://test#{fixture}").to_return(
        status: 200, body: File.open(File.join(fixture_paths[0], "#{fixture}_add_sync.xml"))
      ).times(2).then.to_return(
        status: 200, body: File.open(File.join(fixture_paths[0], "#{fixture}_ordered_sync.xml"))
      )
    end
    ...
    stub_request(:get, "https://testpreservica/api/entity/content-objects/ae328d84-e429-4d46-a865-9ee11157b485/generations/1/bitstreams/1/content").to_return(
      status: 200, body: File.open(File.join(fixture_paths[0], "preservica/api/entity/content-objects/ae328d84-e429-4d46-a865-9ee11157b485/generations/1/bitstreams/1/content.tif"), 'rb')
    )
  ...
  context 'user with permission' do
    ...
    it 'can recognize when child objects are reordered in preservica' do
      ...
      expect(co_first.last_preservica_update).not_to eq nil
