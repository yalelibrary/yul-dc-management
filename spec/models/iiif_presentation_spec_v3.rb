...
RSpec.describe IiifPresentationV3, prep_metadata_sources: true do
  ...
  describe 'building a manifest' do
    it "does have the search service section if the parent_object full_text? is true" do
      ...
      service = iiif_presentation_no_labels.manifest["service"].first
      expect(service['id']).to eq("http://localhost:3000/catalog/2005512/iiif_search")
      expect(service['type']).to eq("SearchService1")
      expect(service['service']['id']).to eq("http://localhost:3000/catalog/2005512/iiif_suggest")
      expect(service['service']['type']).to eq("AutoCompleteService1")
    end
