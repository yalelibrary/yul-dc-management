...
RSpec.describe ActivityStreamLogsDatatable, type: :datatable, prep_metadata_sources: true do
  ...
  describe 'admin set data tables' do
    it 'can handle an empty model set' do
      output = ActivityStreamLogsDatatable.new(datatable_sample_params(columns)).data

      expect(output).to eq([])
    end

    it 'can handle a populated set' do
      output = ActivityStreamLogsDatatable.new(datatable_sample_params(columns),
