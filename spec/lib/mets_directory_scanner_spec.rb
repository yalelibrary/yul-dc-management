...
RSpec.describe MetsDirectoryScanner do
  ...
  it "will scan a directory and create jobs with progress file" do
    expect(BatchProcess).to receive(:new).exactly(2).times.and_return(nil)
    described_class.perform_scan
    expect(File.exist?("#{ENV['GOOBI_MOUNT']}/001/#{described_class.indicator_file_prefix}.progress")).to be_truthy
    expect(File.exist?("#{ENV['GOOBI_MOUNT']}/002/#{described_class.indicator_file_prefix}.progress")).to be_truthy
  end

  it "will skip directory with done file" do
    File.new("#{ENV['GOOBI_MOUNT']}/001/#{described_class.indicator_file_prefix}.done", "w")
    expect(BatchProcess).to receive(:new).exactly(1).times.and_return(nil)
    described_class.perform_scan
    expect(File.exist?("#{ENV['GOOBI_MOUNT']}/002/#{described_class.indicator_file_prefix}.progress")).to be_truthy
  end
  ...
  it "will remove old progress file and process" do
    progress_file = "#{ENV['GOOBI_MOUNT']}/002/#{described_class.indicator_file_prefix}.progress"
    File.new(progress_file, "w")
    File.utime(Time.now.utc, Time.now.utc - 5.days, progress_file)
    expect(BatchProcess).to receive(:new).exactly(2).times.and_return(nil)
    described_class.perform_scan
    expect(File.exist?("#{ENV['GOOBI_MOUNT']}/001/#{described_class.indicator_file_prefix}.progress")).to be_truthy
    expect(File.exist?("#{ENV['GOOBI_MOUNT']}/002/#{described_class.indicator_file_prefix}.progress")).to be_truthy
