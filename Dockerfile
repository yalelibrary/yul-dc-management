FROM yalelibraryit/dc-base:newvips
...
RUN apt-get update && apt-get install -y libtiff-tools liblcms2-dev libexif-dev libmagickcore-dev imagemagick libexpat-dev libtiff5-dev libgsf-1-dev libjpeg-turbo8-dev vim openjdk-11-jre-headless libcfitsio8 libimagequant0 libmatio9 libopenslide0 liborc-0.4-0 libpoppler-glib8 libwebpdemux2 \
  &&  apt-get clean && \
...
COPY vips_8.11.4_amd64.deb $APP_HOME
RUN dpkg -i ./vips_8.11.4_amd64.deb
RUN vips --version
