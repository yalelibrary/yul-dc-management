...
class ParentObject < ApplicationRecord # rubocop:disable Metrics/ClassLength
  ...
  validates :preservica_uri, presence: true, format: { with: %r{\A/}, message: " in incorrect format. URI must start with a /" }, if: proc { digital_object_source.include?("Preservica") }
