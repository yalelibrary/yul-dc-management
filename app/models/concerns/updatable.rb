...
module Updatable
  ...
  def update_parent_objects
    ...
    parsed_csv.each_with_index do |row, index|
      ...
      trigger_setup_metadata(parent_object)
      # Only sync from Preservica if we're not already in a Preservica sync process
      sync_from_preservica if (parent_object.digital_object_source == 'Preservica' || parent_object.digital_object_source == 'preservica') && batch_action != 'resync with preservica'
