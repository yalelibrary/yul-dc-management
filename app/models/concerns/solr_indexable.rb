...
module SolrIndexable
  ...
  def solr_index
    begin
      ...
      result = solr.commit
      (result&.[]("responseHeader")&.[]("status"))&.zero? ? processing_event("Solr index updated", "solr-indexed") : processing_event("Solr index after manifest generation failed", "failed")
    rescue => e
