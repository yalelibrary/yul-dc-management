...
class DelayedJobDashboardController < ApplicationController
  def index
    res = GoodJob::Job.connection.execute("select count(*) as total,
  ...
  def failed_jobs
    @jobs = GoodJob::Job.page(params[:page]).where("last_error is not null").order("run_at desc")
    @job_types = "Failed"
    render :jobs
  end

  def working_jobs
    @jobs = GoodJob::Job.page(params[:page]).where("locked_at is not null and failed_at is null").order("run_at desc")
    @job_types = "Working"
    render :jobs
  end

  def pending_jobs
    @jobs = GoodJob::Job.page(params[:page]).where("attempts=0 and locked_at is null").order("run_at desc")
    @job_types = "Pending"
    render :jobs
  end
  ...
  def delete_job
    GoodJob::Job.find(params[:id]).destroy!
    flash[:notice] = "Job #{params[:id]} deleted."
    redirect_to dashboard_path
  end

  def requeue
    GoodJob::Job.find(params[:id]).update!(run_at: Time.now.utc, last_error: nil)
