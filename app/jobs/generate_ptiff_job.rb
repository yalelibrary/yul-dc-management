...
class GeneratePtiffJob < ApplicationJob
  ...
  def perform(child_object, batch_process)
    ...
    child_object.convert_to_ptiff!(child_object.current_batch_process&.batch_action == 'recreate child oid ptiffs')
    # Only generate manifest if all children are ready
    GenerateManifestJob.perform_later(child_object.parent_object, child_object.current_batch_process, child_object.current_batch_connection) if child_object.parent_object.needs_a_manifest?
  end
end
